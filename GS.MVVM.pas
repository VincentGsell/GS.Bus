...
Public
  ...
  PropertyName : String;
  Prop : TRttiProperty; //TODO : Inherited ?
...
Public
  constructor Create(aSource, aDestination : TGSMVVMItem); reintroduce;
...
Private
  ...
  //message process.
  Procedure InternalDeclarationChannel(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);
  procedure InternalDeclarationLink(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);

  procedure InternalDeclareActivity(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);

  Procedure MVVMProcess(aInstance : string);
    Procedure DoJob; //synchronized.

  //Utility
  procedure InternalRttiProcess(aLink: TGSMVVMLink); //To run in a synchronized part.
  ...
  ///
  //message process.
  Procedure InternalForeignDeclarationChannel(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);
...
protected
  ...
  procedure InternalIncomingCall(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop); Virtual;
public
...
TGSMVVMResponse = Record
  ...
  Procedure CallBack(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);
end;
...
Public
  ...
  Function Link(Const aRessourceId_Source, aRessourceId_Target: String) : Boolean;

  //Up to date CSV : List all ressources availables.
  Function Ressources : String;
...
begin
  ...
  FForeignDeclaration := Subscribe('MVVMForeignDeclare', InternalForeignDeclarationChannel);
end;
...
begin
  ...
  begin
    Sys.BusExecute;
    BusProcessMessages([FDeclaration,Fdeclarationb,Factivity]);
  end;
end;

procedure TGSMVVMEngine.InternalRttiProcess(aLink: TGSMVVMLink);
...
begin
  ...
  lvalok := false;
  if aLink.FRessourceA.Prop<>nil then
  begin
    lval := aLink.FRessourceA.Prop.GetValue(la);
    lvalok := true;
  end
  else
  for lprop in FContext.GetType(la.ClassType).GetProperties do
  begin
    if UpperCase(lprop.name) = upperCase(aLink.FRessourceA.PropertyName) then
    begin
      aLink.FRessourceA.Prop := lProp; //Assignation : Next time, it will pass in above if.
      lval := aLink.FRessourceA.Prop.GetValue(la);
      lvalok := true;
      break;
    end;
  ...
  if lvalok then
  begin
    if aLink.FRessourceB.Prop<>nil then
    begin
      aLink.FRessourceB.Prop.SetValue(lb,lval.ToString);
    end
    else
    for lprop in FContext.GetType(lb.ClassType).GetProperties do
    begin
      if UpperCase(lprop.name) = upperCase(aLink.FRessourceB.PropertyName) then
      begin
        aLink.FRessourceB.Prop := lprop;
...
procedure TGSMVVMEngine.InternalDeclarationChannel(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);
var lObjView : String;
...
procedure TGSMVVMEngine.InternalDeclarationLink(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);
...
begin
  ...
  begin
    lLink := TGSMVVMLink.Create(la,lb);
    FLink.Add(lLink);

    //first level dead Loop detection : Todo : Make a better one, detect path.
    for I := 0 to FLink.Count-1 do
      for j := 0 to FLink.Count-1 do
        if i<>j then
        begin
          if (FLink[i].RessourceA = FLink[j].RessourceB) And (FLink[i].RessourceB = FLink[j].RessourceA) then
            raise Exception.Create('Loop linked ressource (Links indexes '+IntToStr(i)+' and '+intToStr(j)+') ['+FLink[i].RessourceA.RessourceID+'] <-> ['+FLink[j].RessourceA.RessourceID+']');
        end;
  end;
end;

procedure TGSMVVMEngine.InternalDeclareActivity(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);
...
procedure TGSMVVMEngine.InternalForeignDeclarationChannel(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);
...
Function BuildDeclareMessageLink(Const aResA, aResB : String) : TBusMessage;
var lStream : TMemoryStream;
begin
...
procedure TGSMVVMResponse.CallBack(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);
var lStream : TMemoryStream;
    lOutObj : String;
    lMetric : String;
begin
  ...
  try
    ...
    begin
      case lc.Event.WaitFor(INFINITE) of
        wrSignaled :
        begin
         BusProcessMessages([lc]); //Trig lResponse.CallBack (TGSMVVMResponse.CallBack)
         Result := True;
         break;
        end
        else
...
Function TGSMVVM.Link(const aRessourceId_Source, aRessourceId_Target: String) : Boolean;
var la : TBusMessage;
begin
  la := BuildDeclareMessageLink(aRessourceId_Source, aRessourceId_Target);
  GSMVVMEngine.Send(la,'MVVMDeclareLink');
  Result := True;
end;

function TGSMVVM.Ressources: String;
begin
//  GSMVVMEngine.FList
end;
...
constructor TGSMVVMLink.Create(aSource, aDestination : TGSMVVMItem);
begin
  Inherited Create;
  Assert(assigned(aSource));
  Assert(assigned(aDestination));
  Assert(aSource<>aDestination);
  //Is aB yet linked to aA ? and vice versa ? And via other ? (Cascade !!)
  FRessourceA := aSource;
  FRessourceB := aDestination;
end;
...
begin
  ...
  FSharedEvent := GSMVVMEngine.GetNewEvent;
end;

destructor TGSMVVMNest.Destroy;
var i : Integer;
    Ll : TList_TBusClientReader;
begin
  FClientList.Lock;
  try
    Ll := FClientList.GetLockedList;
...
begin
  while Not(Terminated) do
  begin
    case FSharedEvent.WaitFor(CST_BUSTIMER) of
      wrSignaled :
      begin
        BusProcessMessages(FClientList.ToArray);
       //Shared Event : See perhaps : http://seanbdurkin.id.au/pascaliburnus2/archives/230
      end;
    end;
  end;
end;
...
begin
  ...
  try
    ...
    l := GSMVVMEngine.Subscribe(aRessourcePOP,InternalIncomingCall);
    l.Event := FSharedEvent;
    Ll.Add(l);
...
procedure TGSMVVMNest.InternalIncomingCall(Sender : TBusSystem; aReader : TBusClientReader; Var Packet : TBusEnvelop);
...
begin
  ...
  FClientList.Lock;
  try
    lList := FClientList.GetLockedList;
...
constructor TGSMVVMItem.Create;
begin
  Prop := nil;
end;
